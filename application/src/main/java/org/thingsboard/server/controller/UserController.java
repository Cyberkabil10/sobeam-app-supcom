...
public class UserController extends BaseController {
    ...
    public User saveUser(
            ...
            HttpServletRequest request) throws ThingsboardException {
                  ...
                  if(user.getId() == null) {
                 // Vérifie combien d'utilisateurs existent déjà pour ce tenant
                 long numberUser = userService.countByTenantId(tenantId);
                // Si c'est le premier utilisateur, on active (status=true), sinon on désactive (status=false)
                user.setStatus(numberUser == 0);}
        ...
        if (!savedUser.getStatus()) {
            DefaultRole(savedUser);
        }
        return savedUser;
    }
    ...
    public PageData<UserEmailInfo> getUsersForAssign(
        ...
        if (Authority.TENANT_ADMIN.equals(currentUser.getAuthority())) {
            if (alarm.getCustomerId() == null) {
                pageData = userService.findTenantAdmins(getCurrentUser() ,tenantId, pageLink);
            } else {

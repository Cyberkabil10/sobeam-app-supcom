...
public class DefaultTbCalculatedFieldService extends AbstractTbEntityService implements TbCalculatedFieldService {
    ...
    @Override
    public PageData<CalculatedField> findAllByTenant(TenantId tenantId, PageLink pageLink) {
        log.trace("Executing findCalculatedByTenantId, tenantId [{}], pageLink [{}]", tenantId, pageLink);
    ...
    @Override
    public PageData<CalculatedField> findAllByUser(TenantId entityId, UserId userId, PageLink pageLink) {
        log.trace("Executing findCalculatedByUser, userId [{}], pageLink [{}]", userId, pageLink);
        Validator.validateId(entityId, id -> INCORRECT_TENANT_ID + id);
        Validator.validateId(userId, id -> "Incorrect userId " + id);
        Validator.validatePageLink(pageLink);
        return calculatedFieldService.findCalculatedFieldsByUserId(entityId, userId, pageLink);
    }
    ...
    private void checkEntityExistence(TenantId tenantId, EntityId entityId) {
        switch (entityId.getEntityType()) {
            case ASSET, DEVICE, ASSET_PROFILE, DEVICE_PROFILE ,USER-> Optional.ofNullable(entityService.fetchEntity(tenantId, entityId))

...
public class CalculatedFieldEntityMessageProcessor extends AbstractContextAwareMsgProcessor {
    ...
    final RuleEngineRpcService rpcService;
     final SmsService smsService;
     final MailService mailService;
     final NotificationCenter notificationCenter;
     final TbClusterService tbClusterService;
    ...
    CalculatedFieldEntityMessageProcessor(ActorSystemContext systemContext, TenantId tenantId, EntityId entityId) {
        ...
        this.rpcService = systemContext.getTbRuleEngineDeviceRpcService();
        this.cfStateService = systemContext.getCalculatedFieldStateService();
        this.smsService = systemContext.getSmsService();
        this.mailService = systemContext.getMailService();
        this.notificationCenter = systemContext.getNotificationCenter();
        this.tbClusterService = systemContext.getClusterService();
    }
    ...
    private void processStateIfReady(CalculatedFieldCtx ctx, List<CalculatedFieldId> cfIdList, CalculatedFieldState state, UUID tbMsgId, TbMsgType tbMsgType, TbCallback callback) throws CalculatedFieldException {
        ...
        try {
            if (ctx.isInitialized() && state.isReady()) {
                ...
                if (state.isSizeOk() &&  ctx.getCalculatedField().isStatus()) {
                    if (!calculationResult.isEmpty()) {

                        switch (ctx.getAction()) {
                            case "SEND_RPC":
                                tbMsgType = TbMsgType.RPC_CALL_FROM_SERVER_TO_DEVICE;
                                if (calculationResult.getResult().fields().next().getValue().asBoolean()) {
                                DeviceId deviceId = new DeviceId(UUID.fromString(ctx.getCalculatedField().getEntityId().getId().toString()));
                                    cfService.pushMsgToRuleEngine(tenantId, deviceId, ctx, cfIdList ,callback);
                                }
                                break;
                            case "SEND_SMS":
                                String[] phone = new String[]{ctx.getCalculatedField().getConfiguration().getOutput().getPhone()};
                                String sms = ctx.getCalculatedField().getConfiguration().getOutput().getBodySms();
                                smsService.sendSms(tenantId , null , phone ,sms);
                                break;

                            case "SEND_EMAIL":
                                String email = ctx.getCalculatedField().getConfiguration().getOutput().getEmail();
                                String subject = ctx.getCalculatedField().getConfiguration().getOutput().getSubject();
                                String body = ctx.getCalculatedField().getConfiguration().getOutput().getBodyEmail();

                                mailService.sendEmail(tenantId  ,email ,subject,body );
                                break;

                            case"SEND_NOTIFICATION":

                                List<UUID> targets = ctx.getCalculatedField().getConfiguration().getOutput().getTargets();
                                NotificationTemplateId templateId = ctx.getCalculatedField().getConfiguration().getOutput().getTemplate();
    ...
    private String getServiceId() {
        ...
        return "calculatedField-service-" + UUID.randomUUID();
    }

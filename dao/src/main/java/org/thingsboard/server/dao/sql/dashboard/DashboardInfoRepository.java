...
public interface DashboardInfoRepository extends JpaRepository<DashboardInfoEntity, UUID> {
    ...
    @Query("SELECT di FROM DashboardInfoEntity di " +
            "JOIN GroupRelationEntity egr ON di.id = egr.entityId " +
            "JOIN GroupEntity eg ON egr.groupId = eg.id " +
            "WHERE di.tenantId = :tenantId " +
            "AND eg.userIdAffected = :userId " +
            "AND eg.type = 'DASHBOARD' " +
            "AND (:searchText IS NULL OR ilike(di.title, CONCAT('%', :searchText, '%')) = true)")
    Page<DashboardInfoEntity> findByTenantUserId(@Param("tenantId") UUID tenantId, @Param("userId") UUID userId,
                                                 @Param("searchText") String searchText,
                                             Pageable pageable);

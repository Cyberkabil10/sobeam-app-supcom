...
public interface CustomerRepository extends JpaRepository<CustomerEntity, UUID>, ExportableEntityRepository<CustomerEntity> {
    ...
    @Query("SELECT c FROM CustomerEntity c " +
            "JOIN GroupRelationEntity egr ON c.id = egr.entityId " +
            "JOIN GroupEntity eg ON egr.groupId = eg.id " +
            "WHERE c.tenantId = :tenantId " +
            "AND eg.userIdAffected = :userId " +
            "AND eg.type = 'CUSTOMER' " +
            "AND (:textSearch IS NULL OR ilike(c.title, CONCAT('%', :textSearch, '%')) = true)")
    Page<CustomerEntity> findByTenantId(@Param("tenantId") UUID tenantId, @Param("userId") UUID userId,
                                        @Param("textSearch") String textSearch,
                                        Pageable pageable);

...
public class BaseCalculatedFieldService extends AbstractEntityService implements CalculatedFieldService {
    ...
    public CalculatedField save(CalculatedField calculatedField) {
        ...
        try {
            ...
            createOrUpdateCalculatedFieldLink(tenantId, savedCalculatedField);
             eventPublisher.publishEvent(SaveEntityEvent.builder().tenantId(savedCalculatedField.getTenantId()).entityId(savedCalculatedField.getId()).entity(savedCalculatedField).oldEntity(oldCalculatedField).created(calculatedField.getId() == null).build());
    ...
    @Override
    public PageData<CalculatedField> findCalculatedFieldsByUserId(TenantId tenantId, UserId userId, PageLink pageLink) {
        log.trace("Executing findAllByUserId, userId [{}], pageLink [{}]", userId, pageLink);
        validateId(tenantId, id -> INCORRECT_TENANT_ID + id);
        validateId(userId, id -> "Incorrect userId " + id);
        validatePageLink(pageLink);
        return calculatedFieldDao.findAllByUserId(tenantId, userId, pageLink);
    }

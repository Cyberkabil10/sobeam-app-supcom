...
public class BaseEntityService extends AbstractEntityService implements EntityService {
    ...
    @Autowired
    private   GroupService groupDevice;


    @Override
    public long countEntitiesByQuery(UserId userId, TenantId tenantId, CustomerId customerId, EntityCountQuery query) {
        ...
        if (edqsApiService.isEnabled() && validForEdqs(query) && !tenantId.isSysTenantId()) {
            ...
            String type = "ASSET_TYPE".equals(query.getEntityFilter().getType().toString()) ? "ASSET" : "DEVICE";
            boolean hasNoGroups = groupDevice.listGroupUser(tenantId, userId, type).isEmpty();
            if (hasNoGroups) {
            result = entityQueryDao.countEntitiesByQuery(tenantId, null ,customerId, query ,null);
            }else {
            result = entityQueryDao.countEntitiesByQuery(tenantId, userId ,customerId, query, type);
            }
        }
    ...
    public PageData<EntityData> findEntityDataByQuery(UserId userId , TenantId tenantId, CustomerId customerId, EntityDataQuery query) {
        ...
        if (edqsApiService.isEnabled() && validForEdqs(query)) {
            ...
            if (!isValidForOptimization(query)) {
                result = entityQueryDao.findEntityDataByQuery(tenantId, userId, customerId, query , null);
            } else {
                ...
                if (entityDataByQuery == null || entityDataByQuery.getData().isEmpty()) {
                    ...
                    if (hasNoGroups) {
                        ...
                        result = entityQueryDao.findEntityDataByQuery(tenantId, userId , customerId, query , type);
                    }
                }
            }
        }
    ...
    private PageData<EntityData> findEntityIdsByFilterAndSorterColumns(TenantId tenantId, CustomerId customerId, EntityDataQuery query) {
        ...
        return this.entityQueryDao.findEntityDataByQuery(tenantId, null ,customerId, entityQuery,null);
    }
    ...
    private List<EntityData> findEntityDataByEntityIds(TenantId tenantId, CustomerId customerId, EntityDataQuery query,
        ...
        return this.entityQueryDao.findEntityDataByQuery(tenantId, null ,customerId, entityQuery , null).getData();
    }

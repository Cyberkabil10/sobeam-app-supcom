...
public class JpaCalculatedFieldDao extends JpaAbstractDao<CalculatedFieldEntity, CalculatedField> implements CalculatedFieldDao {
    ...
    @Override
    public PageData<CalculatedField> findAllByUserId(TenantId tenantId, UserId userId, PageLink pageLink) {
        log.debug("Try to find calculated fields by userId[{}] and pageLink [{}]", userId, pageLink);
        return DaoUtil.toPageData(calculatedFieldRepository.findAllByUserId(userId.getId(), DaoUtil.toPageable(pageLink)));
    }

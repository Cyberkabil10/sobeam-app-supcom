...
public class DefaultEntityQueryRepository implements EntityQueryRepository {
    ...
    @Override
    public long countEntitiesByQuery(TenantId tenantId,  UserId userId , CustomerId customerId, EntityCountQuery query ,String type) {
        ...
        if ((query.getKeyFilters() == null || query.getKeyFilters().isEmpty()) && type == null) {
            ...
            String reqJoint  = "";
            if (type != null) {
                reqJoint += " JOIN entity_group_relation egr ON e.id = egr.entity_id " +
                        "JOIN entity_group eg ON egr.group_id = eg.id ";

                entityWhereClause += " AND eg.user_id_affected = :user_id AND eg.type = :type ";
                ctx.addUuidParameter("user_id", userId.getId());
                ctx.addStringParameter("type", type);

            }

            String fromClauseCount = String.format("from (select %s from (select %s from %s e %s where %s) entities %s ) result %s",
                    "entities.*",
    ...
    public PageData<EntityData> findEntityDataByQueryInternal(EntityDataQuery query) {
        return findEntityDataByQuery(null, null,null, query, true  , null);
    }

    @Override
    public PageData<EntityData> findEntityDataByQuery(TenantId tenantId, UserId userId , CustomerId customerId, EntityDataQuery query , String type) {
        return findEntityDataByQuery(tenantId, userId , customerId, query, false ,type);
    }

    public PageData<EntityData> findEntityDataByQuery(TenantId tenantId,UserId userId, CustomerId customerId, EntityDataQuery query, boolean ignorePermissionCheck , String type) {
            return transactionTemplate.execute(status -> {
            EntityType entityType = resolveEntityType(query.getEntityFilter());
            ...
            String fromClauseData = String.format("from (select %s from (select %s from %s e  %s where %s) entities %s ) result %s",
                    topSelection,
                    entityFieldsSelection,
                    addEntityTableQuery(ctx, query.getEntityFilter()),
                    reqJoint,

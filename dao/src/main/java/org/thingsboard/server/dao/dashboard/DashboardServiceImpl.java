...
public class DashboardServiceImpl extends AbstractEntityService implements DashboardService {
    ...
    @Autowired
    private  final GroupService groupDevice;
...
public PageData<DashboardInfo> findDashboardsByTenantId(TenantId tenantId, User user, PageLink pageLink) {
    ...
    // Vérifie si l'utilisateur a des groupes de type 'DEVICE'
    if (user.getStatus()) {
        return dashboardInfoDao.findDashboardsByTenantId(tenantId.getId(), pageLink);
    } else {
        return dashboardInfoDao.findDashboardsByTenantUserId(tenantId.getId(),  user.getId().getId(), pageLink);
    }
}

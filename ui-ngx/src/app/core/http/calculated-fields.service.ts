...
export class CalculatedFieldsService {
  ...
  public getTenantCalculatedField(pageLink: PageLink, config?: RequestConfig): Observable<PageData<CalculatedField>> {
    return this.http.get<PageData<CalculatedField>>(`/api/tenant/calculatedField${pageLink.toQuery()}`,
      defaultHttpOptionsFromConfig(config));
  }

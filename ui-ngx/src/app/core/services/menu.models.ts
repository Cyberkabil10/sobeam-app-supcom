...
export const menuSectionMap = new Map<MenuId, MenuSection>([
  ...
  [
    MenuId.calculated_fields,
    {
      ...
      name: 'calculated-fields.management',
      type: 'link',
...
const defaultHomeSectionMap = new Map<Authority, HomeSectionReference[]>([
  ...
  [
    ...
    [
      ...
      {
        name: 'calculated-fields.management',
        places: [MenuId.calculated_fields]
      },
      {
        name: 'feature.management',
        places: [MenuId.features]
      },
      {
        name: 'notification.management',
        places: [MenuId.notifications_center, MenuId.notification_inbox, MenuId.notification_sent, MenuId.notification_recipients, MenuId.notification_templates, MenuId.notification_rules]
      },
      {
        name: 'mobile.management',
        places: [MenuId.mobile_center, MenuId.mobile_apps, MenuId.mobile_bundles, MenuId.mobile_qr_code_widget]
      },
      {
...
export const buildUserMenu = (authState: AuthState, res: any): Array<MenuSection> => {
  const refs = defaultUserMenuMap.get(authState.authUser.authority);
  if (!refs) return [];

  const mapRefs = (refs: MenuReference[]) =>
    refs.map(r => referenceToMenuSection(authState, r)).filter(Boolean);
  ...
  // Set des permissions
  const perms = res?.menuPermission ? new Set(Object.keys(res.menuPermission)) : null;
  if (!perms?.size) {
    return mapRefs(refs);
  }

  // Préparer les sets de lookup une seule fois
  const NOTIFICATION_SUBMENUS = SUBMENUS.get('notifications_center') ?? [];
  const EDGE_SUBMENUS = SUBMENUS.get('edge_management') ?? [];

  const hasAccess = (id: string): boolean => {
    if (id === 'home') return true;

    // Direct mapping
    const directPerm = MENU_TO_PERMISSION.get(id);
    if (directPerm && perms.has(directPerm)) return true;

    // Dépendances composites
    const deps = COMPOSITE_DEPS.get(id);
    if (deps?.some(dep => perms.has(dep))) return true;

    // Sous-menus (test direct, pas besoin de Set)
    if ((NOTIFICATION_SUBMENUS.includes(id) && perms.has('NOTIFICATION')) || 
      (EDGE_SUBMENUS.includes(id) && perms.has('EDGE'))) {
      return true;
    }
  ...
  const filterRefs = (refs: MenuReference[]): MenuReference[] =>
    refs.reduce<MenuReference[]>((acc, r) => {
      if (!hasAccess(r.id)) return acc;

      const pages = r.pages?.length ? filterRefs(r.pages) : undefined;

      // Si composite vide → ignorer
      if (COMPOSITE_DEPS.has(r.id) && (!pages?.length)) {
        return acc;
      }

      acc.push({ ...r, ...(pages ? { pages } : {}) });
      return acc;
    }, []);

  return mapRefs(filterRefs(refs));
};
...
// CORRECTION du mapping des permissions
const PERMISSION_MENU_MAP = new Map<string, string[]>([
  ['ALARM', ['alarms']],
  ['DASHBOARD', ['dashboards']],
  ['DEVICE', ['devices']],
  ['ASSET', ['assets']],
  ['ENTITY_VIEW', ['entity_views']],
  ['GATEWAY', ['gateways']],
  ['RULE_CHAIN', ['rule_chains']],
  ['API_USAGE_STATE', ['api_usage']],
  ['EDGE', ['edge_management', 'edge_instances']],
  ['TB_RESOURCE', ['resources', 'resources_library', 'widget_library', 'widget_types', 'widgets_bundles']], // TB_RESOURCE donne accès aux widgets aussi
  ['CUSTOMER', ['customers']],
  ['ROLE', ['role_management']],
  ['CALCULATED_FIELD', ['calculated_fields']],
  ['SCHEDULEREVENT', ['scheduler']],
  ['OTA_PACKAGE', ['otaUpdates']],
  ['VERSION_CONTROL', ['version_control']],
  ['NOTIFICATION', ['notifications_center']],
  ['MOBILE_APP', ['mobile_center']],
  ['MOBILE_APP_BUNDLE', ['mobile_bundles']],
  ['MOBILE_APP_SETTINGS', ['mobile_apps']],
  ['WIDGET_TYPE', ['widget_types']],
  ['WIDGET_BUNDLE', ['widgets_bundles']],
  ['OAUTH2_CLIENT', ['oauth2']],
  ['AUDIT_LOG', ['audit_log']],
  ['TENANT', ['tenants']],
  ['QUEUE', ['queues']],
  ['DEVICE_PROFILE', ['device_profiles']],
  ['ASSET_PROFILE', ['asset_profiles']],
  ['JS_LIBRARY', ['javascript_library']],
  ['IMAGE', ['images']],
  ['SCADA_SYMBOL', ['scada_symbols']]
]);


// menuId → permission (direct map pré-calculé)
const MENU_TO_PERMISSION = new Map<string, string>(
  [...PERMISSION_MENU_MAP.entries()].flatMap(([perm, menuIds]) =>
    menuIds.map(menuId => [menuId, perm] as [string, string])
  )
);

// Composite menus
const COMPOSITE_DEPS = new Map<string, string[]>([
  ['entities', ['DEVICE', 'ASSET', 'ENTITY_VIEW', 'GATEWAY']],
  ['profiles', ['DEVICE_PROFILE', 'ASSET_PROFILE']],
  ['features', ['OTA_PACKAGE', 'VERSION_CONTROL', 'SCHEDULEREVENT', 'CALCULATED_FIELD']],
  ['security_settings', ['OAUTH2_CLIENT', 'AUDIT_LOG']],
  ['widget_library', ['WIDGET_TYPE', 'WIDGET_BUNDLE']],
  ['resources', ['TB_RESOURCE', 'JS_LIBRARY', 'IMAGE', 'SCADA_SYMBOL']],
  ['notifications_center', ['NOTIFICATION']],
  ['mobile_center', ['MOBILE_APP', 'MOBILE_APP_BUNDLE', 'MOBILE_APP_SETTINGS']],
  ['edge_management', ['EDGE']]
]);

...
export class EntityAutocompleteComponent implements ControlValueAccessor, OnInit {
  ...
  notFoundEntities = this.translate.instant("entity.no-entities-text") ;
    ...
    if (this.entityTypeValue) {
      switch (this.entityTypeValue) {
        ...
        case EntityType.GROUP:
          this.entityText = 'dashboard.group';
          this.noEntitiesMatchingText = this.translate.instant("dashboard.no-groups-matching");
          this.entityRequiredText = this.translate.instant('dashboard.group-required');
          this.notFoundEntities = this.translate.instant('dashboard.no-groups-text');
          break;
  ...
  private fetchEntities(searchText?: string): Observable<Array<BaseData<EntityId>>> {
    ...
    return this.entityService.getEntitiesByNameFilter(targetEntityType, searchText,
      50, this.entitySubtypeValue, { ignoreLoading: true }).pipe(
        catchError(() => of(null)),
        map((data) => {
          if(targetEntityType === EntityType.GROUP || targetEntityType === EntityType.USER){
          switch(this.entityFilter){
              case EntityType.DASHBOARD:
                data = data.filter(entity => (entity as any).type === EntityType.DASHBOARD);
                break;
              case EntityType.ASSET:
                data = data.filter(entity => (entity as any).type === EntityType.ASSET);
                break;
                case EntityType.DEVICE:
                data = data.filter(entity => (entity as any).type === EntityType.DEVICE);
                break;
                case EntityType.CUSTOMER:
                data = data.filter(entity => (entity as any).type === EntityType.CUSTOMER);
                break;
                case EntityType.GROUP:
                if (targetEntityType === EntityType.USER) {
                  data = data.filter(entity => (entity as any).authority === Authority.TENANT_ADMIN);
                }
                break;
              default:
                // no filter applied

    ...
    return this.entityService.getEntitiesByNameFilter(
      ...
      map((data) => {
        ...
        switch (this.entityFilter) {
          ...
          case 'DEVICE':
            return data.filter(entity => (entity as any).type === 'DEVICE');
          case 'DASHBOARD':
            return data.filter(entity => (entity as any).type === 'DASHBOARD');
          case 'ASSET':
            return data.filter(entity => (entity as any).type === 'ASSET');
          case 'CUSTOMER':
            return data.filter(entity => (entity as any).type === 'CUSTOMER');
          case 'TENANT':
            return data.filter(entity => (entity as any).type === 'TENANT');
          default:
            return data;
        }
      })

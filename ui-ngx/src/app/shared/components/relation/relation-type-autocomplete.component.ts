...
writeValue(value: string | null): void {
  ...
  this.relationTypeFormGroup.get('relationType').setValue(value, {emitEvent: false});
  // Force refresh display
  if (value) {
    setTimeout(() => {
      this.displayRelationTypeFn(value);
    });
  }
}
...
displayRelationTypeFn = (relationType: string): string => {
  return relationType ? this.translate.instant(this.relationTypeMap[relationType] || relationType) : '';
};
 ...
 fetchRelationTypes(searchText?: string): Observable<Array<string>> {
  return of(RelationTypes).pipe(
    map(types => {

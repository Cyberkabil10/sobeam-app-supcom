...
import { DashboardId, EntityId } from '@app/shared/public-api';
...
export class DashboardsTableConfigResolver {
  ...
  configureCellActions(dashboardScope: string): Array<CellActionDescriptor<DashboardInfo>> {
    ...
    if (this.authUser?.status) {
        actions.push(
          {
            name: this.translate.instant('device.assign-to-group'),
            icon: 'diversity_2',
            isEnabled: () => true,
      ...
      actions.push(
        {
          name: this.translate.instant('device.assign-to-group'),
          icon: 'diversity_2',
          isEnabled: this.authUser.status,
          onAction: ($event, entities) => this.assignToGroup($event, entities.map((entity) => entity.id))
        },
        {
  ...
  manageAssignedGroups($event: Event, dashboard: DashboardInfo) {
    const assignedCustomersIds = dashboard.assignedCustomers ?
      dashboard.assignedCustomers.map(customerInfo => customerInfo.customerId.id) : [];
    this.showManageAssignedCustomersDialog($event, [dashboard.id.id], 'manage', assignedCustomersIds);
  }
  ...
  manageGroup($event: Event, entityId: EntityId) {
    if ($event) {
      $event.stopPropagation();
    }
  ...
  assignToGroup($event: Event, dashboardIds: Array<DashboardId>) {
    if ($event) {
      $event.stopPropagation();
    }
  ...
  onDashboardAction(action: EntityAction<DashboardInfo>): boolean {
    switch (action.action) {
      ...
      case 'manageAssignedCustomers':
        ...
        return true;
      case 'manageAssignedGroups':
        this.manageGroup(action.event, action.entity.id);
        return true;
  ...
  saveAndAssignDashboard(dashboard: DashboardSetup): Observable<Dashboard> {
    ...
    return this.dashboardService.saveDashboard(dashboardToCreate as Dashboard).pipe(
      mergeMap((createdDashboard) => {
        return this.affecterDashboardAuxGroupes(dashboard, createdDashboard).pipe(
          mergeMap(() => {
  ...
  affecterDashboardAuxGroupes(dashboard: any, saveDashboard: Dashboard): Observable<Dashboard> {
    const listOfGroups = dashboard.groupId;
    if (listOfGroups && listOfGroups.length > 0) {
      const assignObservables = listOfGroups.map((groupId: string) => {
        const groupRelation: GroupRelation = {
          ...
          entityId: { entityType: EntityType.DASHBOARD, id: saveDashboard.id.id }
        };
        return this.entityGroupService.assignEntityToGroup(groupRelation);
      });
      return forkJoin(assignObservables).pipe(map(() => saveDashboard));
    }
    return of(saveDashboard);
  }

...
export class AssetsTableConfigResolver {
  ...
  constructor(private store: Store<AppState>,
    ...
    this.config.saveEntity = asset => {
      return this.assetService.saveAsset(asset).pipe(
        ...
        mergeMap((savedAsset) =>
          this.affecterAssetAuxGroupes(asset, savedAsset).pipe(
            mergeMap(() => this.assetService.getAssetInfo(savedAsset.id.id))
          )
        )
      );
    };
  ...
  assignToGroup($event: Event, assetIds: Array<AssetId>) {
    if ($event) {
      $event.stopPropagation();
    }
  ...
  affecterAssetAuxGroupes(asset: any, saveAsset: Asset): Observable<Asset> {
    const listOfGroups = asset.groupId;
    if (listOfGroups && listOfGroups.length > 0) {
      const assignObservables = listOfGroups.map((groupId: string) => {
        const groupRelation: GroupRelation = {
          ...
          entityId: { entityType: EntityType.ASSET, id: saveAsset.id.id }
        };
        return this.entityGroupService.assignEntityToGroup(groupRelation);
      });
      return forkJoin(assignObservables).pipe(map(() => saveAsset));
    }
    return of(saveAsset);

...
export class MobilePageItemRowComponent implements ControlValueAccessor, OnInit, Validator {
  ...
  get itemName(): string {
    if (this.isDefaultMenuItem) {
    return this.translate.instant(this.defaultItemName);
  }
  const label = this.mobilePageRowForm.get('label')?.value as string | null;
  const translated = label ? this.translate.instant(label) : '';
  return translated || (label ?? '');
  }

  get itemNamePlaceholder(): string {
     return this.isDefaultMenuItem ? this.translate.instant(this.defaultItemName) : '';
  }

    ...
    valueSetter.setValue(value).subscribe({
      ...
      error: (err) => {
        ...
        if (setValueObserver?.error) {
          ...
          const uuid = this.ctx.defaultSubscription?.targetEntityId;
          const $injector = this.ctx.$scope?.$injector;
          const attributeService = $injector.get(this.ctx.servicesMap.get('attributeService'));

          attributeService.getEntityAttributes(uuid, AttributeScope.SHARED_SCOPE, ["state"]).subscribe({
            next: (attributes) => {
              const stateAttr = attributes.find(attr => attr.key === "state");
              const stateValue = stateAttr?.value;

              switch (stateValue) {
                case 0:
                  this.ctx.showErrorToast("Échec de l'action", 'bottom', 'center', this.ctx.toastTargetId, true);
                  break;
                case 1:
                  this.ctx.showSuccessToast("Action envoyée avec succès", 5000, 'bottom', 'center', this.ctx.toastTargetId, true);
                  break;
                case 2:
                  this.ctx.showSuccessToast("L'action a déjà été envoyée",4000, 'bottom', 'center', this.ctx.toastTargetId);
                  break;
                default:
                  this.ctx.showErrorToast("État inconnu", 'bottom', 'center', this.ctx.toastTargetId, true);
                  break;
              }
            },
            error: (fetchErr) => {
              this.ctx.showErrorToast("Erreur de récupération d'état", 'bottom', 'center', this.ctx.toastTargetId, true);
            }
          });

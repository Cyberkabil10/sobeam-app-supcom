  ...
  for (const columnDef of Object.keys(this.columnFilters)) {
    ...
    if (selectedValues.length) {
      ...
      if (key) {
        ...
        // Supposons que le type est stocké dans column.valueType ou déterminé d'une autre façon
        const sampleValue = selectedValues[0];
        const valueType = typeof sampleValue;
        let keyFilter: KeyFilter;

        switch (valueType) {
          case 'string':
            const stringPredicate: StringFilterPredicate = {
          ...
          case 'number':
            const numericPredicate: NumericFilterPredicate = {
              type: FilterPredicateType.NUMERIC,
              operation: NumericOperation.EQUAL,
              value: {
                defaultValue: Number(selectedValues[0]), // For numeric, we might want to handle single values
                userValue: undefined,
                dynamicValue: undefined
              }
            };
          ...
          case 'boolean':
            const booleanPredicate: BooleanFilterPredicate = {
              type: FilterPredicateType.BOOLEAN,
              operation: BooleanOperation.EQUAL,
              value: {
                defaultValue: selectedValues[0] === 'true',
                userValue: undefined,
                dynamicValue: undefined
              }
            };
          ...
          default:
            // Default to string filter if type is unknown
            const defaultPredicate: StringFilterPredicate = {
              type: FilterPredicateType.STRING,
              operation: StringOperation.IN,
              value: {
                defaultValue: selectedValues.join(','),
                userValue: undefined,
                dynamicValue: undefined

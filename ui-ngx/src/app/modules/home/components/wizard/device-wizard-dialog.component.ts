...
export class DeviceWizardDialogComponent extends DialogComponent<DeviceWizardDialogComponent, Device> {
  ...
  constructor(protected store: Store<AppState>,
    ...
    this.deviceWizardFormGroup = this.fb.group({
        ...
        customerId: [null],
        groupId: ['',this.authUser.status === false ? Validators.required : []],
  ...
  private createDevice(): Observable<Device> {
    ...
    if (this.addDeviceWizardStepper.steps.last.completed || this.addDeviceWizardStepper.selectedIndex > 0) {
      return this.deviceService.saveDeviceWithCredentials(deepTrim(device), deepTrim(this.credentialsFormGroup.value.credential)).pipe(
         mergeMap((savedDevice: Device) => this.affecterDevicesAuxGroupes(savedDevice)),
 ...
 affecterDevicesAuxGroupes(device: Device): Observable<Device> {
  const listOfGroups = this.deviceWizardFormGroup.get('groupId').value;
  if (listOfGroups && listOfGroups.length > 0) {
    const assignObservables = listOfGroups.map((groupId: string) => {
      const groupRelation: GroupRelation = {
        ...
        entityId: { entityType: EntityType.DEVICE, id: device.id.id }
      };
      return this.entityGroupService.assignEntityToGroup(groupRelation);
    });
    return forkJoin(assignObservables).pipe(map(() => device));
  }
  return of(device);
}
